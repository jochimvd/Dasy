...
const Home: Component = () => {
  ...
  const fetchAnalysis = async (month: number) => {
    const res = await api.post(`analyses/monthly`, { json: { month: month, year: 2022 } }).json<Response>();
  ...
  createEffect(() => {
    ...
    const data = {
      ...
      },
      {
        ...
        backgroundColor: analysis2()?.map(c => c.priority).map(c => colorFunction(c, 0.2)),
        borderColor: analysis2()?.map(c => c.priority).map(colorFunction),
        borderWidth: 1
      }]
    };
    ...
    const cats = analysis()?.filter(c => c.priority > 20).sort((a, b) => b.priority - a.priority);
  ...
  return (<>
    ...
    Month 1: <input type="number" placeholder="month1" value={month1()} onChange={updateMonth1()} min="1" max="12" />
    <br />
    Month 2: <input type="number" placeholder="month2" value={month2()} onChange={updateMonth2()} min="1" max="12" />
